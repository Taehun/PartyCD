...
WoW 전투 로그 파일을 브라우저에서 직접 읽어 공격대 쿨다운을 실시간 표시하는 정적 웹 뷰어. WoW 12.0 Secret Values 제약으로 인게임 API만으로는 정확한 공대 CD 추적이 어려운 환경에서 로그 파일 tail을 통해 대체합니다.

## 요구사항

- **Chromium 기반 브라우저** (Chrome, Edge, Arc, Brave, Whale 등). Firefox/Safari는 File System Access API 미지원으로 동작하지 않음.
- WoW 게임 내에서 `/combatlog` 으로 전투 로그 파일 생성.

## 사용법

### 1. 정적 서버로 실행

`file://` 경로에서는 File System Access API가 제한되므로 HTTP(S) 컨텍스트가 필요.
...
또는 배포된 [라이브 뷰어](https://taehun.github.io/RaidCD/)를 바로 사용.
...
추적 대상 스펠은 `spells.js`에 정의. 각 스펠은 Wowhead CDN에서 공식 아이콘을 로드하며, 로딩 실패 시 2자 약어로 폴백.
...
- 쿨다운 값은 `baseCD` 고정. 탈레트/헤이스트 보정 없음.
...
파서 단위 테스트 (`test/parser.test.js`) — `SPELL_CAST_SUCCESS` / `SPELL_AURA_APPLIED` / `SPELL_INTERRUPT` 외에 `UNIT_DIED` / `ENCOUNTER_START` / `ENCOUNTER_END` / `SPELL_DAMAGE` 도 다룬다.
...
viewer/
├── index.html
├── app.js           # 폴더 감시, 상태 관리, 렌더링 (Blob.stream() 기반 line streamer)
├── parser.js        # 전투 로그 라인 파서 (순수 함수)
├── spells.js        # 추적 스펠 데이터 (Wowhead 아이콘 + 한글 명칭)
├── style.css
├── test/
│   ├── parser.test.js
│   └── fixtures/
...
3. 파일 크기 증가분을 `Blob.stream() + TextDecoderStream`으로 청크 스트리밍 (대용량 로그 OOM 방지)
